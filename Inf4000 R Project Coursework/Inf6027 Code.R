...
trackData <- read_delim("tracks.csv", delim = "\t")
...
combinedSongData <- merge(combinedSongData, trackData, by = "song_id")
...
acousticFeaturesPopularityGenresDF <- combinedSongData %>%
  select(acousticness, danceability, energy, instrumentalness, liveness, loudness, speechiness, valence, tempo, popularity, genres, release_date) %>%
...
#Capitalise Collumn and Row Names
colnames(cor_matrix) <- str_to_title(colnames(cor_matrix))
rownames(cor_matrix) <-str_to_title(rownames(cor_matrix))

# Visualize the correlation matrix
ggcorrplot(cor_matrix, title = "Pearson's R of all Popularity and Acoustic Features of Spotify Songs",
           lab = TRUE,
           lab_size = 3,
...
coefficientsLongSignificant <-coefficientsLong

#combine the significance and coefficient columns
coefficientsLongSignificant$Coefficient <- paste0(coefficientsLongSignificant$Coefficient, coefficientsLongSignificant$Significant)



#create a matrix of coefficients, with the x axis being predictor variables, and the y axis being outcome variables.
ggplot(coefficientsLongSignificant, aes(x = Predictor, y = Outcome)) +
  geom_tile(fill = "#98FF98", color = "white") +
  geom_text(aes(label = Coefficient), color = "black", size = 5)  +  # Add coefficient values
  labs(
